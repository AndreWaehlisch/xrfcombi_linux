...
void convcomp(multicomplayer *multicomplay, multilayer *multil)
{
  ...
  if (setlagen==NULL)
  {
     printf("mem exhausted in malloc for setlagen in convcomp\n");
     exit(1);
  }
...
/*    printf("Count of nr of elements in whole layer is %d \n", j_elem); /* */  
    ...
    if (reeks==NULL)
    {
      printf("mem exhausted in malloc for reeks in convcomp\n");
      exit(1);
    }
   ...
   if (setlagen[i]==NULL)
   {
     printf("mem exhausted in malloc for setlagen[i] convcomp\n");
     exit(1);
   }
