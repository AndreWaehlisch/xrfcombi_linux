...
int func(struct pstruct *parin)
{
   ...
   double currthickn, otherthickn , corr_ins ;
...
/*       printf("na labda_in \n"); /* */
       ...
       { 
         ...
         primint = pilabda(j, i, pcurrlayer)*
                   corr_ins*(pntrspectra[j][i]).intens; 

         secint = intrasecflu(el,lin, labda_in , &sample, k, primint);

         /* debugging  
         printf("laag %d primint %g  secint %g\n",
                k, primint, secint);  /* */
...
/*  printf("%s %s rxi_cal is %g  meas %g \n", el, lin,
