...
*
*       Original by By J.A. Rupley, Tucson, Arizona
*
*       Adapted by M.Bos
*
*       November 1997
...
int simpfit(FILE *fptr, int (*func)(struct pstruct *parin))
{
        int j, i ;
	int opcode ;
	int test_break ;
...
/*              printf("na bsort \n");   /* later debugging */
		...
		centroid(&pbar, p_p, (nvert - 1)) ;
                /* debugging  
                printf("in simplex na centroid  pbar is\n");  
                show_pars(&pbar); /* pa0mbo */
		...
		if (pstar.val < p_p[nlow]->val){
			...
			func(&p2star) ;
/*            printf("in simplex na ptrail het gamma \n"); /* */
			...
			if (pstar.val <= p_p[nhigh]->val) {
				...
				opcode = HIGHCONTRACT ;
/* printf("In simplex voor 3e ptrail \n");  */

			ptrial(&p2star, p_p[nhigh], &pbar, beta, (1-beta)) ;
/*                        show_pars(&p2star);  /* pa0mbo */

			func(&p2star) ;
/* printf("In simplex na 3e ptrial \n");  /* debug */                       
...
/* reached without exception later weg */
...
/*		if (KBHIT) {
			...
			if ((c == CTRLX) || (c == CTRLC))
				...
				/* print results */
		fprintf(fptr, "\n%5d %20.14e  %s\n",
			iter, p_p[nhigh]->val,  opname[opcode]) ; 
  
        if ( opcode != SHRINK )
		   fprintf(fptr," pbest.val %20.14e \n",p_best->val);
 
/* printf("Na eerste printf\n"); */

		fpprint(fptr, p_p[nhigh]) ;
        if ( opcode != SHRINK )
			fpprint(fptr, p_best) ;
/* exit(0);                not reached now later */
...
/*		test = rms_func/mean_func ; */
                rms_data = (p_p[nlow]->val) ; /* /(ndata - nfree)); */
                test= mean_func; /* pa0mbo's vinding */
...
int simpdev(FILE *fptr, int (*func)(struct pstruct *parin) )
{
	int i, j, k, l ;
	int xfree, free_cnt ;
	int err_count ;
	...
	extern double 		qmat[NPARM][NPARM] ;   /* pa0mbo */
...
/*        printf("Na func(&pcent) \n"); */
	...
	for (i = 0; i < nparm; i++) {
		dtemp = 0 ;
		for (j = 0; j < nvert; j++)
			dtemp = dtemp + fabs(p[j].parm[i] - pcent.parm[i]) ;
		dtemp = dtemp/nvert ;
		if (fabs(dtemp/pcent.parm[i]) < 1.e-16) {
	...
	ypmin = pmin.val ;
				...
				/* calculate 
				standard deviations */
        mse = pcent.val / ( nfree-1) ;  /* pa0mbo was ndata -nfree */
	rms_data = sqrt(mse) ;
	for (i = 0; i < nfree; i++) {
		for (j = 0; j < nfree; j++) {
			qmat[i][j] = qmat[j][i] 
...
int centroid(struct pstruct *pdest, struct pstruct **psrc, int xvert)
{
	int i, j ;
...
void fpprint(FILE *fptr, struct pstruct *g)
{
	int i ;
...
void fsprint(FILE *fptr, char *string)
{
	int j ;
...
void bsort(int nvert)
{
    ...
    while ( ready)
    {
     ...
     /* debugging pa0mbo 
  printf("Debug info sorted ssq's \n");
  for (i=0; i< nvert; i++)
  {
    printf("Val %19.10e  Pars: ",p_p[i]->val);
    for (j=0; j < nparm; j++)
     printf(" %19.10e ",p_p[i]->parm[j]);
    printf("\n");
  } /* */
/*  printf("\n");  */
}
...
*    show_pars()
*
*    debugging routine shows results in a
*    parameter structure for a vertex
*
************************************************/
    
void show_pars(struct pstruct *p)
{
   int i ;
   extern int nfree;
   for (i=0; i < nfree ; i++)
