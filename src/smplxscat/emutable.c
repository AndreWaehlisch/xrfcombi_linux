...
static
void calc_coefs(emupair *pair,emupair prev) {
double kevbig, kevsml, musml, mubig ;
  mubig=pair->abscoef;
  kevbig=pair->energy;
  musml=prev.abscoef;
  kevsml=prev.energy;
  pair->slope=( log(mubig) - log(musml))/(log(kevsml)-log(kevbig));
  pair->A=log(mubig) + pair->slope*log(kevbig);
}
...
double calc_muilambda(int z, double kev, emudata *pemudata)
{
  ...
  pemus= ((*pemudata).atomcoefs[z-1])->paren;
  while ( pemus->energy < kev ) 
	  pemus++;
  return exp(pemus->A-pemus->slope*log(kev)); 
}
                   ...
                   emudata *pemudata)
{
     ...
     for (i=0,cur=(*laag).elementen; i < nr_elems ; i++, cur++)
		mulabda += calc_muilambda(get_z( cur->name, lijst), energy, pemudata)*cur->wfract;
     return(mulabda);
}
    

double calc_mumix_snel(int line_nr, int index,
                        layer *laag, elem_list *lijst,
                   emudata *pemudata)
{
     int i, z, nr_elems ;
