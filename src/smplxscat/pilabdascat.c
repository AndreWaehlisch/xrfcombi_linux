...
*	sep 1997
*
*    corrected for use edges in stead of lambdai for test
*    if line is fluoresced
*
*    adapted to use with smplx Jan 22nd 2004
*
*/
#include <stdio.h>
#include <math.h>
...
double primintfull_scat( double Ci, double Jilambda, double omegai,
...
double pilabdascat(char *elsymb, char *linesymb, double lambda, layer *player)
{
      double  Ci, Jilambda ;
    ...
    /*  printf(" lambdai %g  lambda %g \n",lambdai, lambda); /* */
     ...
     /* if (lambdai < lambda)  return (0.0); /* line is not fluoresced not correct ? nov2003*/
	  ...
	  /* here now test if line is fluoresced Nov 2003 */
	if (lambda > (12.396/edges[z][hole_nr-1])) return( 0.0) ;/* line is not fluoresced */
...
/*	  printf("mu1i is %g \n", mu1i);  /* */
      ...
      pil= primintfull_scat(Ci,Jilambda, omegai, gi, tauilambda,
                       mu1lambda, mu1i);
      pil=pil*selfattn(mu1lambda,mu1i,thickness); 
   /*  printf("return reached in pilabda with pil is %g at lambda %g\n", pil,lambda);    /* */
      return(pil);
}

void show_res_scat(int z, double Ci, char *shellname, int hole_nr,
