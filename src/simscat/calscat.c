...
*
*
* Copyright (C) 2004, M.Bos, for details see copying
*
*                    calcscat.c
*                    Program for quantitative 
*                    X-ray fluorescence analysis
*                    calibration of measured intensities
*                    versus calculated rel.intensities
*                    based on fundamental parameter approach
*                    includes double interactions with
*                    scattering
*
*                    Jan 19th 2004
*
...
int  main(int argc, char *argv[])
{
   ...
   double raylfotint, raylfotint_bulk;
   double comptonint, comptonint_bulk;
   double fotraylint, fotraylint_bulk;
   ...
   double *sigmxy;
   double *sigmxx ;
   double *calib_slope, *sigma2_slope, kmin;
   init_tables();
   /* debugging
 ...
 /* printf("Voor pilamba i is %d en intens %g \n",i, (pntrspectrum[i]).intens);  /* debugging */
  ...
  /*   printf("Give name of file with measured intensities ");
   ...
   fpsmpl=own_fopen(filenaam);
   fpcalres=fopen("./data/calibres.dat","w");
   if (fpcalres==NULL)
   {
  ...
  printf("Na lezen intensiteiten nr_smpls is %d\n", nr_smpls);  /* */
   ...
   /* calculate stdev of slope */
   
   /* omitted for use in xrfcombi */
