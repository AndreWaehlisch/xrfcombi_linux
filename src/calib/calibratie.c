...
*
*
* Copyright (C) 1999, M.Bos, for details see copying
*
*                    calibratie.c
...
*                    februari 1998
*
*                    corrected feb 99 for dimenensions of
*                    malloc sigmxx etc.
*                    added calcn of rel stdev of slopes
*
*************************************************************************/
#include <stdio.h>
#include <string.h>
...
/* printf("Voor pilamba i is %d en intens %g \n",i, (pntrspectrum[i]).intens);  /* debugging */
 ...
 /*      printf("totint %s %s is %e bulkint %e\n",el, lin, totint,totint_bulk);
  ...
  printf("Na lezen intensiteiten nr_smpls is %d\n", nr_smpls);  /* */
   ...
   /* debug 
   printf("sigmxy %g sigmxx %g slope[%d] %g \n", sigmxy[j],
                   sigmxx[j],j,
                   calib_slope[j]); /* */
    fprintf(fpcalres,"%e\n",calib_slope[j]);
   }
